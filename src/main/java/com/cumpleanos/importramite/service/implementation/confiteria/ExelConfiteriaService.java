...
public class ExelConfiteriaService {
    ...
    public byte[] generarExcelConfiteria(ReposicionRequest request) {
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            // Crear estilos UNA sola vez (límite ~64k en POI)
            CellStyle styleProveedor = estiloProveedorStyle(workbook);
            CellStyle styleEmpresa   = estiloEmpresaStyle(workbook);
            CellStyle styleTitulo    = estiloTituloStyle(workbook);
            CellStyle styleFecha     = estiloFechaStyle(workbook);
            ...
            rowNum = crearFilaEncabezado(sheet, rowNum, totalCols,
                    proveedor.toUpperCase(), styleProveedor, 18);

            // --- EMPRESA (fila 1) --------------------------------------------
            rowNum = crearFilaEncabezado(sheet, rowNum, totalCols,
                    EMPRESA, styleEmpresa, 13);
            ...
            for (ConfiteriaDetalle detalle : request.detalles()) {
                ...
                setCellValue(row, 2, detalle.getProNombre(),           wrapStyle);
                setCellValue(row, 3, String.valueOf(detalle.getPedido()), dataStyle);
            }

            // --- ANCHO DE COLUMNAS (fijo, más rápido que autoSizeColumn) -----
            sheet.setColumnWidth(0, 20 * 256); // CODIGO_BARRA
            sheet.setColumnWidth(1, 10 * 256); // ITEM
            sheet.setColumnWidth(2, 40 * 256); // DESCRIPCION  ← la larga
            sheet.setColumnWidth(3, 10 * 256); // PEDIDO
    ...
    // -------------------------------------------------------------------------
    // BUG FIX: se guarda currentRow ANTES de incrementar,
    // y el merge usa ese valor, no el ya incrementado.
    // -------------------------------------------------------------------------
    private int crearFilaEncabezado(Sheet sheet,
                                    int rowNum, int totalCol,
                                    String text, CellStyle style, int altoPuntos) {

        int currentRow = rowNum; // ← guardar ANTES de cualquier incremento
        ...
        if (totalCol > 1) {
            sheet.addMergedRegion(
                    new CellRangeAddress(currentRow, currentRow, 0, totalCol - 1)
            );
        }

        return currentRow + 1; // ← retorna exactamente la siguiente fila
    }
